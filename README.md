...
## Overview
This repository contains the ROS 2 workspace I used to teach the Kinova Gen3 Lite arm how to pick three 40 mm blocks and stack them into a stable tower. The simulation leverages Gazebo, MoveIt 2, and the official Kinova packages, while the custom `pick_and_place.py` node (under `ros2_ws/src/moveit/moveit/scripts/`) drives the sequencing, gripper control, and planning-scene updates.

### Highlights
- Full ROS 2 Humble workspace with MoveIt 2 configuration for the Gen3 Lite arm and gripper.
- Scriptable tasks (`home`, `retract`, `add_scene`, `pick_place_one`, and `stack`) that can be triggered via `ros2 run` parameters.
- Custom poses and timing that align each grasp/placement, enabling a reliable three-block stack in both Gazebo and RViz.
- Utility script (`spawn_blocks.sh`) and documentation screenshots to debug environments or showcase results.
