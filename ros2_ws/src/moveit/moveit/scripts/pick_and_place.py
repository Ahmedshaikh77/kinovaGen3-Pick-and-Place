...
Quick start:
  ...
  ros2 run moveit pick_and_place --ros-args -p task:=add_scene

  # Then, run the main task
  ros2 run moveit pick_and_place --ros-args -p task:=pick_place_one
"""
import time
import threading
import rclpy
...
class PickAndPlaceNode(Node):
    def __init__(self):
        ...
        place_xyz = self.blocks_xyz[1]
        self.pre_place_pose = make_pose(place_xyz[0], place_xyz[1], self.approach_height, *self.top_down_orientation)
        self.place_pose     = make_pose(place_xyz[0], place_xyz[1], self.grasp_height + 0.041, *self.top_down_orientation)

        # For stacking blocks, placing block 3 on top of 1 and 2, height += 0.4.
        pick3_xyz = self.blocks_xyz[2]
        self.pre_grasp3_pose = make_pose(pick3_xyz[0], pick3_xyz[1], self.approach_height, *self.top_down_orientation)
        self.grasp3_pose     = make_pose(pick3_xyz[0], pick3_xyz[1], self.grasp_height,    *self.top_down_orientation)

        self.pre_stack_pose = make_pose(place_xyz[0], place_xyz[1], self.approach_height + 0.1, *self.top_down_orientation)
        self.stack_pose     = make_pose(place_xyz[0], place_xyz[1], self.grasp_height + 0.082, *self.top_down_orientation)
    ...
    def task_stack_all(self):
        """Stack 3 blocks"""

        """Place block 1 onto block 2 first."""
        self.get_logger().info("Starting pick and place task for 'block_1'.")
        ...
        self.get_logger().info("Closing gripper to grasp block...")
        self.close_gripper()
        time.sleep(2.0) # Wait for contacts to settle
        self.moveit2.attach_collision_object("block_1", "end_effector_link", self.touch_links)
        time.sleep(0.5)
        ...
        # 3b. Open the gripper and detach the object from the planning scene
        self.get_logger().info("--- READY TO OPEN GRIPPER ---")
        self.open_gripper()
        self.get_logger().info("--- GRIPPER OPENED ---")
        time.sleep(2.0) # Wait for the object to release
        self.moveit2.detach_collision_object("block_1")
        time.sleep(1)
        ...
        """Place block 3 onto block 1 & 2."""
        self.get_logger().info("Starting pick and place task for 'block_3'.")

        # 1. Initial Setup
        self.move_to_joints(self.j_retract)
        self.get_logger().info("--- READY TO OPEN GRIPPER FOR BLOCK3 ---")
        self.open_gripper()
        time.sleep(2.0) # Give the gripper time to open

        # 2. Pick Sequence
        self.get_logger().info("--- PICK SEQUENCE ---")
        # 2a. Move to a "pre-grasp" pose above the block
        if not self.move_to_pose(self.pre_grasp3_pose): return
        # 2b. Move straight down to the "grasp" pose
        if not self.move_to_pose(self.grasp3_pose, cartesian=True): return

        # 2c. Close the gripper and attach the object in the planning scene
        self.get_logger().info("Closing gripper to grasp block...")
        self.close_gripper()
        time.sleep(2.0) # Wait for contacts to settle
        self.moveit2.attach_collision_object("block_3", "end_effector_link", self.touch_links)
        time.sleep(0.5)

        # 2d. Move straight up to the "pre-grasp" pose to lift the block
        if not self.move_to_pose(self.pre_grasp3_pose, cartesian=True): return

        # 3. Place Sequence
        self.get_logger().info("--- PLACE SEQUENCE ---")
        # 3a. Move straight down to the "place" pose (which is the same as the grasp pose)
        if not self.move_to_pose(self.stack_pose, cartesian=True): return
        ...
        self.get_logger().info("--- GRIPPER OPENED ---")
        time.sleep(2.0) # Wait for the object to release
        self.moveit2.detach_collision_object("block_3")
        time.sleep(1)

        # 4. Completion
        # 4a. Move up away from the placed block
        _ = self.move_to_pose(self.pre_stack_pose, cartesian=True)
        # 4b. Return to a final retracted pose
        self.move_to_joints(self.j_retract)

        self.get_logger().info("Stack task completed successfully.")
...
def main(args=None):
    ...
    try:
        ...
        elif task == "stack":          node.task_stack_all()
        else:
